...
public class SaleProductSU21Service_Hickerson {
	...
	public static String printDaySaleReport(PRODUCTSU21_Hickerson a, PRODUCTSU21_Hickerson b) {
		float subtotal = a.getMoney() + b.getMoney();
		float taxAmount = subtotal * .0825f;
		float total = subtotal + taxAmount;
		String dateString = stringDate.substring(0, 2) +"/"+stringDate.substring(2, 4)+"/"+stringDate.substring(4, 8);
		String space = "                            ";
		String receipt_str = FILE 
	...
	public static void main(String[] args) {
		...
		while (file == null) {
			try {
				print("Enter a date in the format of MMDDYYYY");
				stringDate = read.nextLine();
				file = new Scanner(new File("dayFile_"+stringDate + ".txt"));
				int numOfAUnits = 0;
				int numOfBUnits = 0;
				...
				print(printDaySaleReport(productA, productB));
			}
